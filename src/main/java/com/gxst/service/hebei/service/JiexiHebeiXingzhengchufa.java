...
public class JiexiHebeiXingzhengchufa {
	...
	public static void jiexi() {
		List<Map<String, Object>> htmls = db_local.excuteQuery(
				"select icbc_penalty_text,keyword,company_no,company_name from company_url_hb", null);

		for (Map<String, Object> html : htmls) {

			if (html.get("icbc_penalty_text") == null) {
				continue;
			}
			String disposerRealName = ToDBC.tool(html.get("company_name").toString());

			String mainHtml = html.get("icbc_penalty_text").toString();
			...
			for (Element tb : tbs) {
				if (tb.select("th").size() > 0) {
					...
					if (name.contains("行政处罚信息")) {
						JiexiXingzhengchufaxx.jiexi(tb, html, disposerRealName, db_local);
					}
