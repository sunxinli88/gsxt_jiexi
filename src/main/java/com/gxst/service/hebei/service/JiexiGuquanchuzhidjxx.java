...
public class JiexiGuquanchuzhidjxx {
	...
	public static void jiexi() {
		List<Map<String, Object>> htmls = db_local.excuteQuery(
				"select icbc_pledge_text,keyword,company_no,company_name from company_url_hb", null);
			...
			for (Element tb : tbs) {
				if (tb.select("th").size() > 0) {
					...
					if (name.contains("股权出质登记信息")) {
						JiexiGuquanchuzhixx.jiexiGuquanchuzhixx(tb, html, disposerRealName, db_local);
					}
