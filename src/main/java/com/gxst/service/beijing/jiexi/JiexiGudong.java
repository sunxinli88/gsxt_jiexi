...
public class JiexiGudong {
	private static String tbName = "comp_gudongxinxi";
	...
	public static void jiexi() {
		...
		List<Map<String, Object>> htmls = Paramters.db_local.excuteQuery("select gdxx_html,keyword,company_no,company_name from company_beijing", null);
		...
		for (Map<String, Object> html : htmls) {
			...
			for(Element title:titles){
				...
				colValues.add(rs.get(1).text());
				colValues.add(rs.get(0).text());
