...
public class JiexiBg {
	...
	public static void jiexi() {
		...
		List<Map<String, Object>> htmls = Paramters.db_local.excuteQuery("select bgxx_html,keyword,company_no,company_name from company_beijing", null);
		...
		for (Map<String, Object> html : htmls) {
			...
			for(Element title:titles){
				...
				try{
					Paramters.db_local.insert(colNames.toArray(new String[colNames.size()]), colValues.toArray(new String[colValues.size()]), "comp_biangengjilu");
				}catch(Exception e){
