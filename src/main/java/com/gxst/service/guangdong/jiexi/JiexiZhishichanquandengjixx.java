...
public class JiexiZhishichanquandengjixx {
	...
	public static void jiexi() {
		List<Map<String, Object>> htmls = Paramters.db_local.excuteQuery(
				"select company_knowledge_text,keyword,company_no,company_name from company_url_gd", null);
		
		for (Map<String, Object> html : htmls) {

			if (html.get("company_knowledge_text") == null) {
				continue;
			}
			String disposerRealName = ToDBC.tool(html.get("company_name").toString());

			String mainHtml = html.get("company_knowledge_text").toString();
...
//								Paramters.log.info(html.get("company_name").toString() + " 的 comp_zscqczdjxx 参数值大小和列不一样");
