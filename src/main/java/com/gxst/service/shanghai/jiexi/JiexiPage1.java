...
public class JiexiPage1 {
	...
	public static void jiexiText() {
		
		List<Map<String, Object>> htmls = Paramters.db_local
				.excuteQuery("select page1_content,keyword,company_no,company_name from company_sh where id>36", null);
						...
						try {
							...
							for (Element th : ths) {
								if (null != th.nextElementSibling()) {
									...
									if (title.equals("经营期限自")) {
										startDate = th.nextElementSibling().text();
									}
									if (title.equals("经营期限至")) {
						...
						try{
							...
							for(Element tr:trs){
								...
								if(tds.size()>3){
									...
									colNames.add("rjczewy");
									colValues.add(tds.get(4).text());
									
									colNames.add("renjiaochuziriqi");
									colValues.add(tds.get(5).text());
	...
	public static Map<String, Object> relation() {
		...
		List<Map<String, Object>> cols = Paramters.db_local.excuteQuery(
				"select colName,hanghai_name from z_comp_info where tbName = ?",
				new String[] { "comp_gongshangxinxi" });
		for (Map<String, Object> col : cols) {
			if (null != col.get("hanghai_name")) {
				map.put(col.get("hanghai_name").toString(), col.get("colName"));
			}
		}
